...
function initSteps() {
  ...
  // Amount buttons generation based on EduScore
  const amountContainer = document.getElementById('amountButtonsContainer');
  const amountInfoMessage = document.getElementById('amountInfoMessage');
  const eduScore = parseInt(user.eduscore) || 500;
  
  if (amountContainer) {
    let maxAmount = 25;
    let amounts = [5, 10, 15, 20, 25];
    
    if (eduScore >= 650) {
      maxAmount = 50;
      amounts = [10, 20, 30, 40, 50];
      if (amountInfoMessage) {
        amountInfoMessage.innerHTML = `<i class="fa-solid fa-circle-info me-2"></i>Por tu buen EduScore (Nivel Plata/Oro), tu monto máximo es <strong>$50</strong> y tienes descuentos en tasas de interés.`;
        amountInfoMessage.style.backgroundColor = '#f0fdf4';
        amountInfoMessage.style.borderColor = '#bbf7d0';
        amountInfoMessage.style.color = '#15803d';
      }
    }
    
    amountContainer.innerHTML = amounts.map(a => `
      <div class="col" data-amount="${a}">
...
function actualizarResumen() {
  ...
  let tasa  = rates[selectedDays] || 5;
  
  // Apply EduScore discount
  if (eduScore >= 650) {
    tasa = Math.max(1, tasa - 1); // 1% discount
  }
